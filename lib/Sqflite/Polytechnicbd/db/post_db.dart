...
class SqlPolytechnicPostDB {
  ...
  Future<Database> initdb() async {
    ...
    String path = join(documentsDirectory.path, "PolytechnicPost.db");
  ...
  Future<bool> insertdata(PolytechnicSavepost savepost) async {
    final db = await initdb();
    db.insert("MYTable", savepost.toMap());
    return true;
  }

  Future<List<PolytechnicSavepost>> getdata() async {
    final db = await initdb();
    final List<Map<String, Object?>> datas = await db.query("MYTable");
    return datas.map((e) => PolytechnicSavepost.fromMap(e)).toList();
  }
