...
class McqPostlistPage extends StatefulWidget {
  final int categoryid;
  final String categoryname;
  final String sitename;
  const McqPostlistPage({
    Key? key,
  ...
  @override
  _McqPostlistPageState createState() => _McqPostlistPageState();
}

class _McqPostlistPageState extends State<McqPostlistPage> {
          ...
          : LazyLoadScrollView(
              ...
              child: SingleChildScrollView(
                ...
                child: ListView.builder(
                  ...
                  itemCount: post.mcqpostlist.length,
                  itemBuilder: (context, index) {
                    if (index == post.mcqpostlist.length - 1) {
                      if (post.mcqpostlist.length - 1 < 4) {
                        return Container();
                      } else if (post.mcqpostlist.length == previewpostlength) {
                        return Container();
                      } else if (post.mcqpostlist.length != previewpostlength) {
                        return const Center(
                          child: CircularProgressIndicator(),
                    ...
                    return Card(
                      ...
                      child: InkWell(
                        onTap: () {
                          print(post.mcqpostlist[index].link);
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => WebViewPage(
                                  url: post.mcqpostlist[index].link!,
                                ),
                              ));
                          // print(post.mcqpostlist[index].id);
                          // Provider.of<Postprovider>(context, listen: false)
                          //     .getpostdetails(
                                    ...
                                    Container(
                                      ...
                                      child: ClipRRect(
                                        ...
                                        child: CachedNetworkImage(
                                          ...
                                          imageUrl: post.mcqpostlist[index]
